...
namespace OxyPlot
{
	...
	public static class MathRenderingExtensions
	{
				...
				double angle)
		{
			...
			OxySize totalSize = new OxySize(0, 0);

			OxySize tempSize = new OxySize(0, 0);
			...
			while (i < s.Length)
			{
				...
				if (i + 1 < s.Length && s[i] == '^' && s[i + 1] == '{')
				{
					...
					if (i1 != -1)
					{
						...
						if (Math.Abs(size.Width) > Math.Abs(tempSize.Width))
						{
							tempSize.Width = size.Width;
						}

						if (Math.Abs(size.Height) > Math.Abs(tempSize.Height))
						{
				...
				currentX += (tempSize.Width + 2) * cosAngle;
				currentY += (tempSize.Height + 2) * sinAngle;

				// Add superscript/subscript maximum width and height to total.
				totalSize.Width += (tempSize.Width + 2) * cosAngle;
				totalSize.Height += (tempSize.Height + 2) * sinAngle;

				// Zero superscript/subscript size.
				tempSize.Width = 0;
				tempSize.Height = 0;

				var size2 = drawText(currentX, currentY, regularString, fontSize);

				// Add text size to total.
				totalSize.Width += (size2.Width + 2) * cosAngle;
				totalSize.Height += (size2.Height + 2) * sinAngle;

				currentX += (size2.Width + 2) * cosAngle;
				currentY += (size2.Height + 2) * sinAngle;
			}

			totalSize.Width += tempSize.Width;
			totalSize.Height += tempSize.Height;
